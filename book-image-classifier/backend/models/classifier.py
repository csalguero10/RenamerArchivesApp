...
class ImageClassifier:
    ...
    def classify_image(self, image_path, original_filename):
        """Clasifica una imagen, asegurando un valor de retorno incluso si falla."""
        # CORREGIDO: Envolver toda la lógica en un bloque try-except
        try:
            image = cv2.imread(image_path)
            if image is None:
                # Si la imagen no se puede cargar, devolvemos un error claro.
                raise ValueError(f"No se pudo cargar la imagen desde la ruta: {image_path}")
            
            # ... (el resto de tu lógica de clasificación)

        except Exception as e:
            print(f"Error en clasificación para {image_path}: {e}")
            # Devolvemos un tipo por defecto con confianza mínima para no romper el flujo.
    ...
    def _classify_by_filename(self, filename):
        ...
        patterns = {
            ...
            'inserto': [r'\bins\b', r'insert', r'ins'],
            'referencia': [r'\bref\b', r'reference', r'target', r'ref', r'calibr'],
